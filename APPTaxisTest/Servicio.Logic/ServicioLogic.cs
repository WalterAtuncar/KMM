...
namespace Servicio.Logic
{
    public class ServicioLogic : IServicioLogic
    {
        ...
        public async Task<OperationService> EmailService(Correo request)
        {
            ...
            var compania = request.CompaniaUsuario;
            ...
            var operation = await mail.EnviarCorreo(compania,correoResponsable: correoResponsable, correoUsuarioRegistro: correoRegistrador, listaAprobador: listaCorreo, asunto: ConfigurationManager.AppSettings["asunto"].ToString(), mensaje: cuerpo);
            return operation;
        }
        ...
        public async Task<OperationService> EmailService_GV(Correo_GV request)
        {
            ...
            var listaCorreo = new List<string>();
            var listaCorreoCopia = new List<string>();
            var correoRegistrador = (request.Responsable.Email ?? null);
            var correoResponsable = string.Empty;
            var correoCopia = string.Empty;
            var compania = request.CompaniaUsuario;
            //correoResponsable = request.ListaBeneficiado[0].Email;
            //listaCorreoCopia.Add(request.ListaSoporteAdministrativo[0].Email);
            string emailListaSoporteAdministrativo = (request.ListaSoporteAdministrativo != null && request.ListaSoporteAdministrativo.Count > 0)? request.ListaSoporteAdministrativo[0].Email: string.Empty;
            listaCorreoCopia.Add(emailListaSoporteAdministrativo);

            listaCorreo.Add(request.Email);

            string asuntoTipo = ConfigurationManager.AppSettings["asunto_GV"].ToString();
            int idTipoCorreo = request.IdTipoCorreo;

            // Definir las palabras a reemplazar según el tipo de correo
            string palabraReemplazar = "";
            switch (idTipoCorreo)
            {
                case 2:
                    palabraReemplazar = "REEMBOLSO";
                    break;
                case 3:
                    palabraReemplazar = "CAJA";
                    break;
                case 4:
                    palabraReemplazar = "TARJETA";
                    break;
                // Agrega más casos según sea necesario

                default:
                    // Manejar el caso por defecto o lanzar una excepción según tus necesidades
                    break;
            }

            // Reemplazar la palabra "ANTICIPO" con la palabra específica según el tipo de correo
            string asuntoFinal = asuntoTipo.Replace("ANTICIPO", palabraReemplazar);

            var operation = await mail.EnviarCorreo_GV(compania,correoResponsable: correoResponsable, correoUsuarioRegistro: correoRegistrador, listaAprobador: listaCorreo, listaSoporteAdministrativo: listaCorreoCopia, asunto: asuntoFinal, mensaje: cuerpo);
            return operation;
        }
