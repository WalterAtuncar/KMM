...
namespace WebApiAplications.Controllers
{
    ...
    public class IntegracionController : ApiController
    {
        ...
        [Route("getOrdenServicioBatch")]
        [HttpPost]
        public async Task<IEnumerable<Dictionary<string, string>>> GetOrdenServicioBatch(List<OrdenServicioBatch> OrdenServicioBatch)
        {
            Utilitario.Logger();
            try
            {
                ...
                Utilitario.Logger();
                Utilitario.WriteInfoLog($"RFC - Ordenes Batch {JsonConvert.SerializeObject(OrdenServicioBatch)}");
                string json = JsonConvert.SerializeObject(OrdenServicioBatch);
                var FCT = ConfigurationManager.AppSettings["RFC_ORDENES_BATCH"];
                RfcRepository repo = SAP_RFC_Destination.Repository;
                IRfcFunction list = repo.CreateFunction(FCT);
                IRfcTable rfc_tbl = list.GetTable("T_AUFNR_I");

                foreach (OrdenServicioBatch dto in OrdenServicioBatch)
                {
                    rfc_tbl.Append();
                    rfc_tbl.SetValue("AUFNR", dto.ordenservicio);
                   
                }
                list.Invoke(SAP_RFC_Destination);

                IRfcTable rfc_tbl2 = list.GetTable("T_ORDEN_S");
                var sapTable = rfc_tbl2._ToDataTable();
