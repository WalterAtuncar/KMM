...
namespace Service.Logic
{
    public class SolicitudServiceLogic : ISolicitudServiceLogic
    {
        ...
        public async Task<OperationResult> Add(Data.Solicitud element, List<General.Destino> ListaDestino, List<TarifaResponse> ListaTarifa, string usuarioRegistro)
        {
            var xml = new ServicioTransaccionXML();
            var transaction = ObtenerDatosServiceXML(element, ListaDestino, ListaTarifa, usuarioRegistro);
            xml.ServicioXML = ConvertFormat.ObjectToXml(transaction);
            using (var ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    var newId = await oISolicitudService.Add(xml);
                    ts.Complete();
        ...
        public async Task<ServicioRequest> GetServicioRequestBySolicitud(int id, int idProveedor)
        {
            ...
            modelo.ListaDestino = new List<General.Destino>();
            modelo.ListaCliente = new List<General.Cliente>();
            ...
            modelo.Telefono = solicitud.Telefono == null ? "994732829" : solicitud.Telefono;
            ...
            {
                var cliente = new General.Cliente();
